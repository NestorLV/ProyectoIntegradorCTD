...
public class ReservationIntegrationTests {
    ...
    public void crearReserva() throws Exception {
        CategoryDTO categoryDTO = categoryService.save(new CategoryDTO(1,"Apart Hotels", "500.012", "http://exampleproduct.com"));
        CityDTO cityDTO = cityService.save(new CityDTO (1,"Merlo","Argentina"));
        ProductDTO product = productService.save(new ProductDTO("Hotel Carton","Hotel 5 estrellas",93.45,143.67,"Calle Falsa 123, CABA",9,"En el centro",categoryDTO,cityDTO,"rules","health","politics"));
        UserResponseDTO user = userService.save(new UserRequestDTO("Florcita","Landa","tatianalunarincon@gmail.com","1234", new RoleDTO(RolesTypes.USER)));
        ReservationDTO payloadDTO = new ReservationDTO("10:30", DatesConverter.convertLocalDateToSqlDate(LocalDate.of(2021,10,03)), DatesConverter.convertLocalDateToSqlDate(LocalDate.of(2021,10,13)),product.getId(),user.getId());
        ReservationDTO responseDTO = new ReservationDTO(3,"10:30",DatesConverter.convertLocalDateToSqlDate(LocalDate.of(2021,10,03)), DatesConverter.convertLocalDateToSqlDate(LocalDate.of(2021,10,13)),product.getId(),user.getId());

        ObjectWriter writer = new ObjectMapper()
                .configure(SerializationFeature.WRAP_ROOT_VALUE, false)
                .writer();

        String reservationEntity = writer.writeValueAsString(payloadDTO);
        String reservationExpectedResponse = writer.writeValueAsString(responseDTO);

        MvcResult response = mvc.perform(MockMvcRequestBuilders.post("/reservations/create")
                ...
                .andReturn();
        Assert.assertEquals(reservationExpectedResponse, response.getResponse().getContentAsString());
    }*/
